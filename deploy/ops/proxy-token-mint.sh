...
#   3. adds it to the live nave.env; STRIPS it from the consumer
...
if [ -f nave-consumer.env ]; then
  grep -vE '^NACT_PROXY_TOKEN=' nave-consumer.env > nave-consumer.env.tmp
  chmod 600 nave-consumer.env.tmp; mv nave-consumer.env.tmp nave-consumer.env
fi
