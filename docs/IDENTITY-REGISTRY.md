...
| **James's Quill** | The Director's own (personal) Quill instance — his reconnect agent (`quill.md` §9) and drafting hand. **Distinct from the canonical instance.** Two device keys, same persona, no key copied — both are trusted pens on the Ngage desk, independently revocable. | `npub13uuznpc7chk4mlcmve2d8j5832slgvffdre33vvtzk8pmyls5dlsezdfj8` | `8f3829871ec5` | Device-held — Mac Keychain (`WhenUnlockedThisDeviceOnly`); never in SOPS |
| **James's Quill · bunker** | The **always-on device key** for the same persona (warm.contact#48). Generated on the Mac, imported once into Bunker46; signs draft scopes (30440/1059/13 **only**, no kind-1) over a scoped NIP-46 connection for the 24/7 drafter. Cannot sign a post. | `npub1neaatsezuqga0nms5na6gyp8lhhjhtg9yspuw6k3v9qvm7lhdqqsjxffne` | `9e7bd5c322e0` | **Bunker46** (relay/bunker box), AES-256-GCM at rest; the Mac copy was wiped after import. Scoped, rate-capped, unattended-draft connection only |
