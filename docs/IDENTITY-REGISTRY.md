...
| **James's Quill** | The Director's own (personal) Quill instance — his reconnect agent (`quill.md` §9) and drafting hand. **Distinct from the canonical instance.** **One key, sole custody Bunker46** (`quill.md` §9, 2026-07-24): every drafter — box scribe, Mac `warm quill-draft` — borrows signatures over a scoped NIP-46 connection (draft kinds 30440/1059/13 only, no kind-1). Cannot sign a post; the Director's own npub signs those. | `npub13uuznpc7chk4mlcmve2d8j5832slgvffdre33vvtzk8pmyls5dlsezdfj8` | `8f3829871ec5` | **Bunker46** (relay/bunker box), AES-256-GCM at rest, imported 2026-07-24. Interim Mac Keychain copy retires after the Bitwarden escrow step (Phase 5) |
...
| **waggle** | The **bridge's own posting identity** (Buzz nest) — signs delivery into the community on both lanes. Deliberately **lean and disposable**: no sign-off authority, membership only in the channels it routes to, infrastructure only. It holds exactly one key — its own — and no other member's, so a compromise costs a re-mint rather than a person — which is the property the leanness protects | `npub1s36nypljc6h88tey0kshf688eyd8myu636ctfs4e3d2w54nhsmnqfhaent` | `84753207f2c6` | **SOPS** (Director's ruling, 2026-07-30) — the deliberate exception to the Buzz-nest agent-held rule below, because this key sits on a bridge host rather than in a Desktop runtime. A remote-signer (bunker) replacement is the intended direction if it proves feasible |
