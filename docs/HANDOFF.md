...
- PR-body footer (standing rule, Director 2026-07-23): end a PR description with
  `Drafted with assistance from [Claude Code](https://claude.com/claude-code)`.
  The softer, honest wording — **not** `🤖 Generated with…` / `Generated by
  Claude Code`, which is retired. (Distinct from the *post* disclosure line,
  which is enforced in `post-format.mjs`; this one is the git surface.)
