...
| **Nscribe** | The front door on the **`revoice`** actuator (scoped-agent-actions §II·5): paste/upload text → re-voiced in the Director's hand from `brief/jaf.md` (grant-to-app) → handed to Ngage for his signature. Static, no-build, vendors `nave-connect`/`nipxx` | `JAFairweather/nave.pub` (`nscribe/`; graduate to own repo later) | `nscribe.nave.pub` (needs deploy) | 🟡 v1 built — re-voice + preview + Ngage handoff work with a credential; auto-seal (jaf-quill bunker) is the next increment |
