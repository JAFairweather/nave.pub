...
| **waggle** | The **Nostr ↔ Buzz bridge**: non-custodial, quarantine-gated two-way interop between a walled Buzz community and public Nostr. Members federate outward under their own keys; inbound replies land in a default-closed quarantine released by a one-word approval in-channel. Sealed DM and group lanes are carried, never decrypted. Working reference consumer of draft NIP-DA (#2411) for its granted-participant tier. Internal service name `west-bridge` | `JAFairweather/waggle` (local: `~/.buzz/REPOS/waggle`) | `waggle.nave.pub` — the site lives in this repo (`waggle/`); the bridge service itself runs on its own host | 🟢 core-live — all four proof rungs met, including a cold-stranger walk-in held at the quarantine gate |
